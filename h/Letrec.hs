...
data ExpVal = IntVal Int
            ...
            | RecProcVal String String Exp Env
  deriving (Show)
...
valueOf (LetRecExp name arg pbody lbody) env = 
  valueOf lbody $ extendEnv name (RecProcVal name arg pbody env) env
valueOf (CallExp rator rand) envOuter =  
  case valueOf rator envOuter of 
    ...
    e@(RecProcVal name arg body envInner) ->
      let randVal = valueOf rand envOuter
      in valueOf body (extendEnvMany [(arg,randVal),(name,e)] envInner) 

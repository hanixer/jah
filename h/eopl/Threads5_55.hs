...
applyCont cont val = do
  ...
  if timeExp
    then enqueueCurrThread (\_ -> applyCont cont val) >> runNextThread
...
waitCont cont (MutexRef loc) = do
  ...
  if opened
    ...
    else enqueueMutexThread loc (\_ -> applyCont cont EmptyVal) >> runNextThread
...
runNextThread = do
  ...
  case threadQueue sdata of
    ...
    (tid:tids) -> do
      put $ sdata {currTicks = maxTicks sdata, threadQueue = tids, currTId = tid}
      th <- getThread tid
      th ()
...
isMutexQueueEmpty mid = handleMutex mid (\_ tids -> return $ null tids)

openMutex :: MId -> Bool -> InterpM ()
openMutex mid open = handleMutex mid (\_ tids -> updateMutex mid (open, tids))

enqueueMutexThread :: MId -> Thread -> InterpM ()
enqueueMutexThread mid th = do
  sdata <- get
  updateThread (currTId sdata) th
  handleMutex mid (\opened tids -> updateMutex mid (opened, tids ++ [currTId sdata]))

dequeueMutexThread :: MId -> InterpM ()
dequeueMutexThread mid = handleMutex mid (\opened (tid:tids) ->
  updateMutex mid (opened, tids) >> enqueueThread tid)

handleMutex :: MId -> (Bool -> [TId] -> InterpM a) -> InterpM a
handleMutex mid f = do
  sdata <- get
  case IM.lookup mid (mutexes sdata) of

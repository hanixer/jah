...
operandCont :: ExpVal -> Cont -> Cont -> Cont
...
diffCont1 :: Exp -> Env -> Cont -> Cont -> Cont
...
divCont1 :: Exp -> Env -> Cont -> Cont -> Cont
divCont1 e2 env cont econt val1 = 
  evalk e2 env (divCont2 val1 cont econt) econt

divCont2 :: ExpVal -> Cont -> Cont -> Cont
divCont2 val1 cont econt val2 = 
  if unboxInt val2 == 0 
    then econt (IntVal (-1))
    else cont (IntVal $ unboxInt val1 `div` unboxInt val2)

tryNormalCont :: Cont -> Cont
tryNormalCont cont = cont

tryCatchCont :: String -> Exp -> Env -> Cont -> Cont -> Cont
tryCatchCont var catchExpr env cont econt val =
  evalk catchExpr (extendEnv var val env) cont econt

unhandledCont :: Cont
unhandledCont _ = liftIO $ print "Unhandled exception"

evalk :: Exp -> Env -> Cont -> Cont -> InterpM ()
...
evalk (TryExp e1 v e2) env cont econt = 
  evalk e1 env cont (tryCatchCont v e2 env cont econt)
evalk (RaiseExp e) env _ econt = 
  evalk e env econt econt

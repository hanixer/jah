...
letprocExp = do strTok "letproc"
                var1 <- identifier
                args <- (argsListExp identifier)
                e1 <- expr
                strTok "in"
                e2 <- expr
                return (LetExp var1 (ProcExp args e1) e2)

callExp = do 
  procExp <- expr
  params <- (argsListExp expr)
  return (CallExp procExp params)

callExpRest = do
  rands <- (argsListExp expr)
  randss <- do
    randss <- callExpRest
    return randss
   <|> return []
...
valueOf (ProcExp args body) env = (ProcVal args body env)
valueOf (CallExp rator rands) outerEnv =  
  case valueOf rator outerEnv of 
    (ProcVal args body oldEnv) -> 
      let newEnv =
            foldl (\newEnv (var, exp) -> 
                     extendEnv var (valueOf exp outerEnv) newEnv)  
            oldEnv (reverse (zip args rands)) 
      in valueOf body newEnv
...
sampleEnv = (extendEnv "g" (valueOf (fst (head (parse expr "proc (s) -(25,s)"))) emptyEnv)
             (extendEnv "x" (IntVal 10) 
              (extendEnv "v" (IntVal 5)
               (extendEnv "i" (IntVal 1) emptyEnv))))

eval s = valueOf' s sampleEnv

s1 = "let x = 200 in let f = proc (z) -(z,x) in let x = 100 in let g = proc (z) -(z,x) in -((f 1), (g 1))"
s2 = "let x = 200 in letproc f (z) -(z,x) in let x = 100 in letproc g (z) -(z,x) in -((f 1), (g 1))"
s3 = "\
\let f = proc (x) proc(y)\
\  -(x, -(0, y))\
\in ((f 10) 11)"
manyArgCall = "\
\letproc f (x,y,z) +(x,+(y,z))\
\in -(200,(f 1 2 3))"
newCallSyntax = "\
\letproc f (x,y) -(x,y)\
\in f(1,2)"

...
recprocDef = do
  ...
  strTok "="
  pbody <- expr
  return (RecProc name arg pbody)


letrec = do strTok "letrec"
            procs <- (oneAndMore recprocDef id)
            strTok "in"
            lbody <- expr
            return (LetRecExp procs lbody)
...
extendEnvRec recprocs env = Env (\x ->
  case (find (\rp -> (rpName rp) == x) recprocs) of
    (Just rp) -> 
      (ProcVal (rpArg rp) (rpBody rp) (extendEnvRec recprocs env))
    Nothing -> applyEnv env x)
...
valueOf (LetRecExp recprocs lbody) env = 
  valueOf lbody $ (extendEnvRec recprocs env)

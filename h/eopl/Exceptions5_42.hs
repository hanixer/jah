...
letccExp :: Parser Exp
letccExp = strTok "letcc" >> identifier >>= \v -> strTok "in" >> 
  expr >>= \e -> return $ LetccExp v e
...
operandCont (ContVal cont) _ _ val =
  cont val
...
evalk (LetccExp v body) env cont econt = 
  evalk body (extendEnv v (ContVal cont) env) cont econt

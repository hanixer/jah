...
expr :: Parser Exp
expr = numExp <|> letExp <|> letrefExp <|> ifExp <|> procExp <|> letrec <|> callExp <|>
  compoundExp
  <|> isZeroExp
  <|> minusExp
  <|> diffExp
...
eval (LetExp var e1 e2) env = do 
  ...
  eval e2 newEnv
eval (LetRefExp var e1 e2) env = do 
  rhs <- evalOperand e1 env
  case rhs of 
    rv@(RefVal _) -> 
      eval e2 $ extendEnv var rv env
    _ -> throwErr (ErrorData "refval expected")
...
exs = [
  ...
  "let x = 1 in letref y = x in {set y = 33; -(y, x)}",
  "let x = 1 in let y = x in {set y = 33; -(y, x)}"
  ]

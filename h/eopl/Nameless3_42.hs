...
-- trimmed procedure representation - i.e. proc closure captures only used free variables
-- TODO: procedure value lookup elimination
...
data Exp = NumExp Int
          ...
          | NamelessProcExp Exp FreeVarMap
          -- Arithmetic
          | DiffExp Exp Exp
          | MinusExp Exp
          | AddExp Exp Exp
...
extendEnv var val env = ([var]:env)

extendEnv' :: [String] -> Env -> Env
extendEnv' vars env = (vars:env)
...
lexaddrpos :: [String] -> String -> Maybe Int
lexaddrpos vars name = 
...
type FreeVarMap = [Lexaddr] -- represents mapping - from trimmed nameless env to original nameless env
...
freeVars (IfExp e1 e2 e3) bound = 
  foldl union [] (map (\e -> freeVars e bound) [e1,e2,e3])
freeVars (VarExp var) bound = if elem var bound then [] else [var]
freeVars (LetExp var e1 e2) bound = 
  union (freeVars e1 bound) (freeVars e2 (union [var] bound))
freeVars (ProcExp vars body) bound = freeVars body (union vars bound) 
freeVars (CallExp e1 e2) bound = freeVars' ([e1] ++ e2) bound
freeVars (DiffExp e1 e2) bound = freeVars' [e1,e2] bound
freeVars _ bound = []

freeVars' :: [Exp] -> [String] -> [String]
freeVars' exps bound = foldl (\acc x -> union acc (freeVars x bound)) [] exps

freeVarMap :: [String] -> Env -> FreeVarMap
freeVarMap freeVars env = map (\v -> lexaddr env v) freeVars
...
trans (ProcExp args body) env = 
  NamelessProcExp (trans body (extendEnv' args [frVars])) frVarMap
  where frVars = freeVars body args
        frVarMap = freeVarMap frVars env
...
valueOf (NamelessProcExp body fvMap) env = 
  NamelessProcVal body (map (\addr -> [applyNamelessEnv env addr]) fvMap)
...
evalFile file = do
  ...
  print $ evalNl contents--trans (fst $ head $ parse expr contents) emptyEnv

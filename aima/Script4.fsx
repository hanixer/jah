...
let rec newVarName var substs =
    if Map.containsKey var substs then
        newVarName (var + "'") substs
    else var
...
let rec replcaeVarsInTerms substs = function
    ...
    | Var name ->
        ...
        | Some (Some other) -> Var other
        | _ -> Var name
    | term -> term

let rec replaceVars substs fm  =
    let getNewVarsAndSubsts var (newVars, newSubsts)  =
        if Map.containsKey var newSubsts then
            let newVar = newVarName var newSubsts
            (newVar :: newVars, Map.add var (Some newVar) newSubsts)
        else
            (var :: newVars, Map.add var None newSubsts)

    match fm with
    | Forall (vars, fm1) ->
        let (newVars, newSubsts) =
            List.foldBack getNewVarsAndSubsts vars ([], substs)
        let fm1 = replaceVars newSubsts fm1 
        Forall (newVars, fm1)
    | Exists (vars, fm1) ->
        let (newVars, newSubsts) =
            List.foldBack getNewVarsAndSubsts vars ([], substs)
        ...
        Exists (newVars, fm1)
    | Pred (name, terms) -> 
        Pred (name, List.map (replcaeVarsInTerms substs) terms)
    | TermEqual (term1, term2) ->
        TermEqual (replcaeVarsInTerms substs term1, replcaeVarsInTerms substs term2)
    | _ -> transformChildren (replaceVars substs) fm
...
anmls |> eliminateImplications |> moveNotInwards |> (replaceVars Map.empty) |> toString